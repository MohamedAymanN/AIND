...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            actionNode=PgNode_a(action)
            stateSet=self.s_levels[level]
            if actionNode.prenodes.issubset(stateSet):
                for state in stateSet:
                    state.children.add(actionNode)
                    actionNode.parents.add(state)
                actionsList.add(actionNode)
        self.a_levels.append(actionsList)
    ...
    def add_literal_level(self, level):
        ...
        statsList=set()
        actionSet=self.a_levels[level-1]
        for action in actionSet:
            stateSet=action.effnodes
            for state in stateSet:
                state.parents.add(action)
                action.children.add(state)
                statsList.add(state)
        self.s_levels.append(statsList)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        a1Action=node_a1.action
        a2Action=node_a2.action
        
        for eff1 in a1Action.effect_add:
            if eff1 in a2Action.effect_rem:
                return True


        for eff2 in a2Action.effect_add:
            if eff2 in a1Action.effect_rem:
                return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for eff1 in a1Action.effect_rem:
            if eff1 in a2Action.precond_pos:
                return True


        for eff2 in a2Action.effect_rem:
            if eff2 in a1Action.precond_pos:
                return True
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            goalFound=False
            levelsLen=len(self.s_levels)
            level=0
            while(not goalFound and level!=levelsLen):
                currLevel=self.s_levels[level]
